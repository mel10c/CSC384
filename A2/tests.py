...
class TestStringMethods(unittest.TestCase):
    ...
    ########################################
    # Custom test cases and helper function for nary_a_grid function
    def helper_nary_ad_grid(self, board):
        '''Test to ensure that nary_ad_grid correctly sets up all-different constraints for rows and columns.'''
        for board in BOARDS:
            # csp, var_array = nary_ad_grid(board)
            csp, _ = nary_ad_grid(board)
            size = board[0][0]
            all_diff_constraints_row = 0
            all_diff_constraints_col = 0

            # Check if there is exactly one all-different constraint per row and per column
            for con in csp.get_all_cons():
                vars_in_con = con.get_scope()
                if len(vars_in_con) != size:
                    continue  # Skip if not an all-different constraint

                # Check if constraint covers a row
                if all(var.name[1] == vars_in_con[0].name[1] for var in vars_in_con):
                    all_diff_constraints_row += 1

                # Check if constraint covers a column
                if all(var.name[2] == vars_in_con[0].name[2] for var in vars_in_con):
                    all_diff_constraints_col += 1

            expected_constraints = size * 2  # One for each row and one for each column
            actual_constraints = all_diff_constraints_row + all_diff_constraints_col
            self.assertEqual(actual_constraints, expected_constraints, f"Failed to set up correct all-different constraints for nary_ad_grid.  Expected {expected_constraints}, got {actual_constraints}")

    # Tests for nary_ad_grid based on BOARDS
    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Encodings.")
    def test_nary_ad_grid_1(self):
        board = BOARDS[0]
        self.helper_nary_ad_grid(board)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Encodings.")
    def test_nary_ad_grid_2(self):
        self.helper_nary_ad_grid(BOARDS[1])

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Encodings.")
    def test_nary_ad_grid_3(self):
        self.helper_nary_ad_grid(BOARDS[2])

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Encodings.")
    def test_nary_ad_grid_4(self):
        self.helper_nary_ad_grid(BOARDS[3])

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Encodings.")
    def test_nary_ad_grid_5(self):
        self.helper_nary_ad_grid(BOARDS[4])

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Encodings.")
    def test_nary_ad_grid_6(self):
        self.helper_nary_ad_grid(BOARDS[5])
