...
# @max_grade(1)
##Tests FI after the first queen is placed in position 1.
def test_simple_FI(stu_propagators):
    did_fail = False
    score = 0
    try:
        queens = nQueens(8)
        curr_vars = queens.get_all_vars()
        curr_vars[0].assign(1)
        stu_propagators.prop_FI(queens, newVar=curr_vars[0])
        answer = [[1], [3, 4, 5, 6, 7, 8], [2, 4, 5, 6, 7, 8], [2, 3, 5, 6, 7, 8], [2, 3, 4, 6, 7, 8],
                  [2, 3, 4, 5, 7, 8], [2, 3, 4, 5, 6, 8], [2, 3, 4, 5, 6, 7]]
        var_domain = [x.cur_domain() for x in curr_vars]
        for i in range(len(curr_vars)):
            if var_domain[i] != answer[i]:
                details = "Failed simple FI test: variable domains don't match expected results."
                did_fail = True
                break
        if not did_fail:
            score = 1
        ...
        details = "One or more runtime errors occurred while testing simple FI: %r" % traceback.format_exc()

    return score, details


def three_queen_FI(stu_propagators):
    score = 0
    try:
        queens = nQueens(8)
        curr_vars = queens.get_all_vars()
        ...
        stu_propagators.prop_FI(queens)
        answer = [[4], [6, 7, 8], [1], [3, 8], [6, 7], [2, 8], [2, 3, 7, 8], [5]]
        var_vals = [x.cur_domain() for x in curr_vars]

        if var_vals != answer:
            details = "Failed three queens FI test: variable domains don't match expected results"

        else:
            score = 1
            details = ""
    except Exception:
        details = "One or more runtime errors occurred while testing FI with three queens: %r" % traceback.format_exc()
...
def main(stu_propagators=None):
    ...
    print("---starting test_simple_FI---")
    score, details = test_simple_FI(stu_propagators)
    total += score
    print(details)
    print("---finished test_simple_FI---\n")
    ...
    print("---starting three_queen_FI---")
    score, details = three_queen_FI(stu_propagators)
    total += score
    print(details)
    print("---finished three_queen_FI---\n")
